  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-rf-text mb-1 flex items-center gap-2">
            <Users className="w-8 h-8 text-amber-400" />
            {t('admin.users.title')}
          </h1>
          <p className="text-rf-mutedFg text-sm">{t('admin.users.subtitle')}</p>
        </div>
      </div>

      <div className="mb-6">
        <div className="relative max-w-md">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-rf-mutedFg" />
          <input
            type="search"
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder={t('admin.users.searchPlaceholder')}
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-rf-border bg-rf-well/60 text-rf-text placeholder:text-rf-mutedFg focus:outline-none focus:ring-2 focus:ring-amber-500/40"
          />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="card text-center py-20 text-rf-mutedFg">{t('admin.users.empty')}</div>
      ) : (
        <>
          <div className="overflow-x-auto rounded-2xl border border-rf-border bg-rf-well/40">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-rf-border text-rf-text">
                    <td className="px-4 py-3 tabular-nums text-rf-mutedFg">{row.id}</td>
                    <td className="px-4 py-3 font-medium text-rf-text">{row.name}</td>
                    <td className="px-4 py-3 text-rf-mutedFg">{row.email}</td>
                    <td className="px-4 py-3 capitalize text-rf-mutedFg">{row.role}</td>

  ...
  return (
    <div>
      <Link
        ...
        className="inline-flex items-center gap-2 text-sm text-rf-mutedFg hover:text-rf-text mb-6"
      >
        <ArrowLeft className="w-4 h-4" />
        {t('admin.subscriptionPlans.backToList')}
      </Link>

      <h1 className="text-2xl font-bold text-rf-text mb-6">
        {isNew ? t('admin.subscriptionPlans.formTitle') : t('admin.subscriptionPlans.editTitle')}
      </h1>

      <form onSubmit={submit} className="max-w-2xl space-y-4">
        <div>
          <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldSlug')} *</label>
          <input
            className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text"
            value={form.slug}
        ...
        <div>
          <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldName')} *</label>
          <input
            className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text"
            value={form.name}
        ...
        <div>
          <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldDescriptionEn')}</label>
          <textarea
            className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text min-h-[72px]"
            value={form.description_en}
            onChange={(e) => setForm((f) => ({ ...f, description_en: e.target.value }))}
          />
        </div>
        <div>
          <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldDescriptionUk')}</label>
          <textarea
            className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text min-h-[72px]"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldMonthlyCredits')} *</label>
            <input
              type="number"
              min={1}
              className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text"
          ...
          <div>
            <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldPriceUsd')} *</label>
            <input
              className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text"
        ...
        <div>
          <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldFeaturesJson')}</label>
          <p className="text-xs text-rf-mutedFg mb-2">{t('admin.subscriptionPlans.fieldFeaturesJsonHelp')}</p>
          <textarea
            className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text font-mono text-xs min-h-[160px]"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldVariant')}</label>
            <select
              className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text"
          ...
          <div>
            <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldSubscriptionTier')}</label>
            <p className="text-xs text-rf-mutedFg mb-2">{t('admin.subscriptionPlans.fieldSubscriptionTierHelp')}</p>
            <select
              className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text"
          ...
          <div>
            <label className="text-xs text-rf-mutedFg block mb-1">{t('admin.subscriptionPlans.fieldSort')}</label>
            <input
              type="number"
              min={0}
              className="w-full rounded-xl bg-rf-well border border-rf-border px-3 py-2 text-rf-text"
        ...
        <div className="flex flex-wrap gap-6">
          <label className="flex items-center gap-2 text-sm text-rf-mutedFg cursor-pointer">
            <input
              type="checkbox"

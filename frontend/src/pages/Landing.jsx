...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-rf-page">
      <SeoHead
      ...
      <section className="relative z-10 max-w-4xl mx-auto px-6 pt-20 pb-28 text-center">
        ...
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-rf-text leading-tight mb-6 max-w-4xl mx-auto">
        ...
        <p className="text-lg text-rf-mutedFg max-w-2xl mx-auto mb-10 leading-relaxed">
          {t('landing.heroSub')}
        </p>
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-6 pb-24" aria-labelledby="how-it-works-heading">
        <div className="text-center mb-12">
          <h2 id="how-it-works-heading" className="text-3xl sm:text-4xl font-bold text-rf-text mb-3">
            {t('landing.howItWorksTitle')}
          </h2>
          <p className="text-rf-mutedFg max-w-xl mx-auto">{t('landing.howItWorksSubtitle')}</p>
        </div>
          ...
          {howItWorksKeys.map((key, idx) => {
            ...
            return (
              <li key={key} className="relative">
                <div className="h-full rounded-2xl border border-rf-border bg-rf-sidebar p-6 pt-8 text-center md:text-left backdrop-blur-sm hover:border-brand-500/25 transition-colors">
                  ...
                  <h3 className="text-lg font-semibold text-rf-text mb-2">
                    {t(`landing.howItWorks.${key}.title`)}
                  </h3>
                  <p className="text-sm text-rf-mutedFg leading-relaxed">
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-6 pb-24">
        <h2 className="text-3xl font-bold text-rf-text text-center mb-12">
          ...
          {featureKeys.map((key, idx) => {
            ...
            return (
              <div key={key} className="card hover:border-brand-500/30 transition-all duration-200">
                ...
                <h3 className="font-semibold text-rf-text mb-2">{t(`landing.features.${key}.title`)}</h3>
                <p className="text-sm text-rf-mutedFg leading-relaxed">
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-6 pb-24" aria-labelledby="card-examples-heading">
        <div className="text-center mb-12">
          <h2 id="card-examples-heading" className="text-3xl sm:text-4xl font-bold text-rf-text mb-3">
            {t('landing.cardExamplesTitle')}
          </h2>
          <p className="text-rf-mutedFg max-w-xl mx-auto">{t('landing.cardExamplesSubtitle')}</p>
        </div>
        <ProductCardCarousel />
      </section>
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-6 pb-24" aria-labelledby="video-social-heading">
        <div className="text-center mb-12">
          <h2 id="video-social-heading" className="text-3xl sm:text-4xl font-bold text-rf-text mb-3">
            {t('landing.videoSectionTitle')}
          </h2>
          <p className="text-rf-mutedFg max-w-2xl mx-auto leading-relaxed">{t('landing.videoSectionSubtitle')}</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-2xl mx-auto">
          {[t('landing.videoExample1'), t('landing.videoExample2')].map((label, i) => (
            <div
              key={i}
              className="relative mx-auto w-full max-w-[220px] aspect-[9/16] rounded-2xl border border-rf-border bg-gradient-to-b from-rf-well to-rf-page flex flex-col items-center justify-center gap-3 p-6 text-center"
            >
              <Film className="w-10 h-10 text-brand-400/90" aria-hidden />
              <span className="text-sm text-rf-mutedFg leading-snug">{label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="relative z-10 max-w-2xl mx-auto px-6 pb-24 text-center">
        <div className="card border-brand-500/20">
          ...
          <h2 className="text-2xl font-bold text-rf-text mb-3">{t('landing.ctaTitle')}</h2>
          <p className="text-rf-mutedFg mb-6">{t('landing.ctaSub')}</p>
          <Link

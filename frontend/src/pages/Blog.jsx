  ...
  return (
    <div className="min-h-screen bg-rf-page text-rf-text">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 pt-14 pb-12">
        ...
        <p className="text-rf-mutedFg text-lg max-w-xl">{t('blog.subtitle')}</p>
      </div>

      {/* Featured post */}
      <div className="relative z-10 max-w-5xl mx-auto px-6 pb-14">
        <div className={`relative rounded-3xl overflow-hidden bg-gradient-to-br ${featured.gradient} border border-rf-border p-8 md:p-10 group cursor-pointer hover:border-rf-border transition-all duration-200`}>
          <div className="max-w-lg">
            <span className={`inline-flex items-center text-xs font-semibold px-3 py-1 rounded-full border ${featured.tagColor} mb-4`}>
              {featured.tag}
            </span>
            <h2 className="text-2xl md:text-3xl font-bold text-rf-text mb-4 group-hover:text-brand-300 transition-colors">
              {featured.title}
            </h2>
            <p className="text-rf-mutedFg text-sm leading-relaxed mb-6">{featured.excerpt}</p>
            <div className="flex items-center gap-4 text-xs text-rf-mutedFg">
          ...
          <div className="absolute bottom-8 right-8 md:right-10">
            <div className="w-10 h-10 rounded-full bg-rf-elevated border border-rf-border flex items-center justify-center group-hover:bg-brand-600 transition-colors">
              <ArrowRight className="w-4 h-4 text-rf-text" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 pb-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {rest.map((post) => (
            <div
              key={post.id}
              className="group bg-rf-well/60 border border-rf-border hover:border-rf-border rounded-2xl p-6 flex flex-col cursor-pointer transition-all duration-200"
            >
              <div className={`inline-flex items-center self-start text-xs font-semibold px-3 py-1 rounded-full border ${post.tagColor} mb-4`}>
                {post.tag}
              </div>
              <h3 className="font-bold text-rf-text text-base leading-snug mb-3 group-hover:text-brand-300 transition-colors flex-1">
                {post.title}
              </h3>
              <p className="text-rf-mutedFg text-sm leading-relaxed mb-5 line-clamp-3">
                {post.excerpt}
              </p>
              <div className="flex items-center justify-between text-xs text-rf-mutedFg mt-auto">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 pb-24 text-center">
        <div className="bg-gradient-to-r from-brand-900/40 to-purple-900/40 border border-brand-500/20 rounded-3xl p-10">
          ...
          <p className="text-rf-mutedFg text-sm mb-6">{t('blog.newsletter.subtitle')}</p>

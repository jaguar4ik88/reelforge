...
export default function LandingFooter() {
  ...
  return (
    <footer className="relative z-10 border-t border-rf-border py-10 px-6">
      <div className="max-w-6xl mx-auto flex flex-col gap-8">
        <div className="flex flex-col gap-8 sm:flex-row sm:items-start sm:justify-between sm:gap-12">
          <p className="text-rf-mutedFg text-sm text-center sm:text-left order-2 sm:order-1">
            {t('landing.footer', {
          ...
          >
            {footerNav.map(({ to, labelKey }) => (
              <Link key={to} to={to} className="text-rf-mutedFg hover:text-rf-mutedFg transition-colors">
                {t(labelKey)}
              </Link>
            ))}
          </nav>
        </div>
        <p className="text-center text-xs text-rf-mutedFg pt-8 mt-2 border-t border-rf-border">
          {t('landing.developedBy')}{' '}
          <a
            href="https://aidentika.online/"
            target="_blank"
            rel="noopener noreferrer"
            className="text-rf-mutedFg hover:text-brand-400 transition-colors"
          >
            Aidentika Apps
          </a>
        </p>

...
class StorePhotoGenerationRequest extends FormRequest
{
    public function rules(): array
    {
        ...
        $sceneAllowed = $contentType === 'photo'
            ? ['from_wishes', 'no_watermark', 'studio']
        ...
        return [
            ...
            /** Product card HTML layout when Puppeteer text overlay is enabled. */
            'card_html_template' => ['nullable', 'string', Rule::in(['fullbg', 'bold', 'soft', 'banner', 'minimal'])],
            /** Final raster format for card downloads (Puppeteer path). */
            'card_output_format' => ['nullable', 'string', Rule::in(['jpg', 'jpeg', 'png'])],
            'card_price_old' => ['nullable', 'string', 'max:64'],
        ];
    }

    /**
     * Legacy clients may still send scene_style "in_use"; map to "no_watermark".
     */
    protected function prepareForValidation(): void
    {
        if ($this->input('scene_style') === 'in_use') {
            $this->merge(['scene_style' => 'no_watermark']);
        }
    }
